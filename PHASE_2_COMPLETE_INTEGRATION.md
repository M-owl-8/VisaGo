# Phase 2 Complete Integration Guide

## Status
✅ Prisma models added
✅ VisaTemplateService created
✅ Templates expanded for all 10 countries × 2 visa types (20 templates total)
✅ Country code normalization added to ai-context.service.ts

## Remaining: Backend Integration

The file `apps/backend/src/services/ai-openai.service.ts` needs Phase 1 + Phase 2 integration. Due to file stability issues, please apply these changes manually using the guide below.
...
**ADD before `isHybridChecklistEnabled` method:**
...
### STEP 3: Update LEGACY mode section (around line ~1205)
...
**FIND the large inline system prompt and REPLACE WITH:**
```typescript
      // Use frozen system prompt from centralized config - Phase 1
      const systemPrompt = CHECKLIST_SYSTEM_PROMPT;
```

### STEP 5: Update user prompt (around line ~1406-1534)

**FIND the entire user prompt construction block and REPLACE WITH:**
```typescript
...
### STEP 6: Update parsing logic (around line ~1630)

**FIND the parsing block and REPLACE WITH:**
```typescript
...
### STEP 7: Update retry logic (around line ~1685)

**FIND retry block and ensure it also enforces templates after retry.**

### STEP 8: Update final return logic (around line ~1755)

**FIND all references to `parsed.checklist` and `parsed.type`, replace with `legacyChecklist.checklist` and `legacyChecklist.type`.**
...
✅ All 20 templates (10 countries × 2 visa types):
- US: student_long_stay, tourist_short
- GB: student_long_stay, tourist_short
- ES: student_long_stay, tourist_short
- DE: student_long_stay, tourist_short
- JP: student_long_stay, tourist_short
- AE: student_long_stay, tourist_short
- CA: student_long_stay, tourist_short
- AU: student_long_stay, tourist_short
- FR: student_long_stay, tourist_short
- IT: student_long_stay, tourist_short

All templates include realistic core documents with proper isCoreRequired flags.
