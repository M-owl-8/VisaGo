...
class AIOpenAIService {
     ...
     */
    static async callChecklistAPI(messages, options, context) {
        const modelsToTry = [this.CHECKLIST_MODEL, 'gpt-4o', 'gpt-4.1'].filter((model, index, arr) => arr.indexOf(model) === index); // Remove duplicates
        let lastError = null;
        for (let i = 0; i < modelsToTry.length; i++) {
            ...
            (0, logger_1.logInfo)('[Checklist][AI] Attempting checklist generation', {
                model,
                attempt: i + 1,
            ...
            try {
                ...
                (0, logger_1.logInfo)('[Checklist][AI] Successfully generated checklist', {
                    model,
                    country: context.country,
                    visaType: context.visaType,
                    mode: context.mode || 'unknown',
                ...
                (0, logger_1.logWarn)('[Checklist][AI] Model attempt failed', {
                    model,
                    attempt: i + 1,
                    totalAttempts: modelsToTry.length,
                    isTimeout,
    ...
    static initialize(prisma) {
        if (!AIOpenAIService.openai) {
            ...
            if (!apiKey) {
                (0, logger_1.logError)('[OPENAI_CONFIG_ERROR] OPENAI_API_KEY not configured', new Error('Missing OPENAI_API_KEY'), {
                    model: AIOpenAIService.MODEL,
                });
                throw new Error('OPENAI_API_KEY not configured');
            }
            AIOpenAIService.openai = new openai_1.default({
                apiKey,
                timeout: 60000, // 60 second timeout for checklist generation (GPT-4 can take longer)
            });
            AIOpenAIService.prisma = prisma;
            (0, logger_1.logInfo)('[OpenAI] Service initialized', {
    ...
    static async chat(messages, systemPrompt) {
        ...
        if (!AIOpenAIService.isInitialized()) {
            ...
            if (!AIOpenAIService.prisma) {
                ...
                const { default: db } = await Promise.resolve().then(() => __importStar(require('../db')));
...
If you don't know something, say so clearly and suggest how to find the information.`;
     ...
     */
    static async isHybridChecklistEnabled(countryCode, visaType) {
        const { findVisaDocumentRuleSet } = await Promise.resolve().then(() => __importStar(require('../data/visaDocumentRules')));
     ...
     */
    static buildHybridSystemPrompt(country, visaType, visaKb) {
...
${JSON.stringify(baseChecklist.map((item) => ({
...
Return ONLY valid JSON matching the schema, no other text, no markdown, no comments.`;
     ...
     */
    static validateHybridResponse(gptChecklist, baseChecklist) {
        const errors = [];
        const baseDocTypes = new Set(baseChecklist.map((b) => b.documentType));
        const gptDocTypes = new Set(gptChecklist.map((g) => g.documentType || g.document));
        ...
        for (const gptItem of gptChecklist) {
            ...
            if (baseItem) {
                if (gptItem.category !== baseItem.category) {
                    errors.push(`Category mismatch for ${docType}: expected ${baseItem.category}, got ${gptItem.category}`);
                }
                if (gptItem.required !== baseItem.required) {
                    errors.push(`Required mismatch for ${docType}: expected ${baseItem.required}, got ${gptItem.required}`);
                }
            }
        }
        return {
     ...
     */
    static correctHybridResponse(gptChecklist, baseChecklist) {
        const corrected = [];
        const baseMap = new Map(baseChecklist.map((b) => [b.documentType, b]));
        // Process each base document
        for (const baseItem of baseChecklist) {
            const gptItem = gptChecklist.find((g) => (g.documentType || g.document) === baseItem.documentType);
            if (gptItem) {
     ...
     */
    static inferPriorityFromCategory(category) {
        const normalized = category === 'required' || category === 'highly_recommended' || category === 'optional'
            ? category
            : 'optional';
        if (normalized === 'required')
            return 'high';
        if (normalized === 'highly_recommended')
            return 'medium';
        return 'low';
    }
    /**
     ...
     */
    static getDocumentNameFromKB(documentType, country, visaType, visaKb) {
    ...
    static async generateChecklist(userContext, country, visaType) {
        ...
        try {
            ...
            const { getVisaKnowledgeBase } = await Promise.resolve().then(() => __importStar(require('../data/visaKnowledgeBase')));
            const { getRelevantDocumentGuides } = await Promise.resolve().then(() => __importStar(require('../data/documentGuides')));
            ...
            const countryCode = countryCodeMap[normalizedCountry] || country.substring(0, 2).toUpperCase();
            ...
            if (isHybrid) {
                ...
                // ========================================================================
                (0, logger_1.logInfo)('[OpenAI][Checklist] Using HYBRID mode (rule engine + GPT enrichment)', {
                    country,
                    visaType,
                    countryCode,
                });
                const { findVisaDocumentRuleSet } = await Promise.resolve().then(() => __importStar(require('../data/visaDocumentRules')));
                const { buildBaseChecklistFromRules } = await Promise.resolve().then(() => __importStar(require('./checklist-rules.service')));
                const { VisaRulesService } = await Promise.resolve().then(() => __importStar(require('./visa-rules.service')));
                const ruleSetData = await VisaRulesService.getActiveRuleSet(countryCode, visaType);
                if (!ruleSetData) {
                    (0, logger_1.logWarn)('[OpenAI][Checklist] Rule set not found, falling back to legacy mode', {
                        country,
                        visaType,
                        countryCode,
                    });
                    ...
                    if (baseChecklist.length === 0) {
                        ...
                        while (attempt < maxAttempts) {
                            ...
                            try {
                                ...
                                const responseTime = Date.now() - startTime;
                                (0, logger_1.logInfo)('[OpenAI][Checklist] Hybrid mode GPT-4 response received', {
                                ...
                                if (!parsed || !parsed.checklist || parsed.checklist.length === 0) {
                                    (0, logger_1.logWarn)('[OpenAI][Checklist] Hybrid mode parsing failed', {
                                        country,
                                        visaType,
                                        attempt,
                                    ...
                                    const validationResult = this.validateHybridResponse(parsed.checklist, baseChecklist);
                                    if (!validationResult.isValid) {
                                        (0, logger_1.logWarn)('[OpenAI][Checklist] Hybrid mode validation failed', {
                                            country,
                                            visaType,
                                            attempt,
                                        ...
                                        (0, logger_1.logInfo)('[OpenAI][Checklist] Hybrid mode response corrected', {
                                            country,
                                            visaType,
                                ...
                                (0, logger_1.logWarn)('[OpenAI][Checklist] Hybrid mode request error', {
                                    country,
                                    visaType,
                                    attempt,
                                    errorMessage,
                        ...
                        if (parsed && parsed.checklist && parsed.checklist.length > 0) {
                            ...
                            const enrichedChecklist = parsed.checklist.map((item) => {
                                ...
                                return {
                                    ...
                                    required: baseItem?.required !== undefined ? baseItem.required : item.required !== false,
                            ...
                            (0, logger_1.logInfo)('[OpenAI][Checklist] Hybrid mode checklist generation completed', {
                                model: response?.model || this.getChecklistModel(),
                                country,
                                visaType,
                                countryCode,
                        ...
                        // ========================================================================
                        (0, logger_1.logWarn)('[OpenAI][Checklist] Hybrid mode GPT failed after retries, using rule-based fallback', {
                            country,
                            visaType,
                            countryCode,
                            ruleSet: `${countryCode}-${visaType}`,
                        ...
                        const fallbackChecklist = baseChecklist.map((item) => {
                            ...
                            const docName = this.getDocumentNameFromKB(item.documentType, country, visaType, visaKb);
                            return {
                                ...
                                descriptionUz: docName.descriptionUz || `Talab qilinadigan hujjat: ${item.documentType}`,
                                ...
                                whereToObtainUz: 'Tafsilotlar uchun elchixona yoki viza ariza markaziga murojaat qiling.',
                                whereToObtainRu: 'Обратитесь в посольство или центр подачи заявлений на визу для получения подробной информации.',
                            };
                        });
                        (0, logger_1.logInfo)('[OpenAI][Checklist] Rule-based fallback checklist generated', {
                            country,
                            visaType,
                            countryCode,
                            ruleSet: `${countryCode}-${visaType}`,
            ...
            // ========================================================================
            (0, logger_1.logInfo)('[OpenAI][Checklist] Using LEGACY mode (GPT-4 decides documents)', {
                country,
                visaType,
                countryCode,
            });
...
Your goal: produce the most reliable, accurate, embassy-ready checklist every time.`;
            ...
            if (userContext.questionnaireSummary) {
                ...
                if (summary.travelInfo?.duration) {
                    ...
                    if (summary.travelInfo.duration.includes('more_than_6_months') ||
                        summary.travelInfo.duration.includes('more_than_1_year')) {
                        riskFactors.push('Long stay duration - requires stronger proof of ties and financial capacity');
                    }
                }
                ...
                if (summary.financialInfo?.sponsorDetails) {
                    ...
                    riskFactors.push(`Sponsored by ${keyInfo.sponsorType} - include sponsor financial documents`);
                }
                ...
                // Travel history
                if (summary.travelHistory?.traveledBefore === false ||
                    (summary.travelHistory?.visitedCountries &&
                        summary.travelHistory.visitedCountries.length === 0)) {
                    keyInfo.travelHistory = false;
                    riskFactors.push('No previous international travel - include stronger proof of ties to home country');
                }
                else {
                ...
                if (summary.previousVisaRejections || summary.travelHistory?.hasRefusals) {
                    ...
                    riskFactors.push('Previous visa refusal - include explanation letter and stronger supporting documents');
                }
                // Financial situation
            ...
            const promptPayload = {
                country,
                visaType,
                visaKnowledgeBase: visaKb || 'No specific knowledge base available for this country/visa type.',
                documentGuides: documentGuidesText || 'No Uzbekistan-specific document guides matched this request.',
                applicantContext: {
                ...
                instructions: 'Generate a comprehensive checklist with 8-15 items tailored to this specific applicant profile. Consider the risk factors when determining which additional documents to include.',
            };
...
Return ONLY valid JSON matching the schema exactly, no other text, no markdown, no comments.`;
            (0, logger_1.logInfo)('[OpenAI][Checklist] Generating checklist', {
            ...
            try {
                ...
                const errorMessage = openaiError?.message || String(openaiError);
                (0, logger_1.logWarn)('[OpenAI][Checklist] All checklist model attempts failed', {
                    country,
                    visaType,
                    errorMessage: errorMessage.substring(0, 200),
                    timeoutMs: 60000,
            ...
            if (responseTime > 30000) {
                (0, logger_1.logWarn)('[OpenAI][Checklist] Slow response', {
                    country,
                    visaType,
                    responseTimeMs: responseTime,
            ...
            (0, logger_1.logInfo)('[OpenAI][Checklist] Raw GPT-4 response received', {
                model: response?.model || this.getChecklistModel(),
                country,
                visaType,
                responseLength: rawContent.length,
            ...
            const { getFallbackChecklist } = await Promise.resolve().then(() => __importStar(require('../data/fallback-checklists')));
            let parsed = null;
            let validationResult = null;
            let attempt = 1;
            let needsRetry = false;
            // First attempt
            const firstAttempt = parseAndValidateChecklistResponse(rawContent, country, visaType, attempt);
            ...
            if (!validationResult.isValid) {
                (0, logger_1.logWarn)('[OpenAI][Checklist] Attempt 1 failed validation', {
                    country,
                    visaType,
                    errors: validationResult.errors,
            ...
            if (needsRetry && attempt < 2) {
                (0, logger_1.logInfo)('[OpenAI][Checklist] Attempt 2: Retrying with stricter JSON requirements', {
                    country,
                    visaType,
...
5. No markdown, no comments, no extra text outside JSON`;
                ...
                const retryAttempt = parseAndValidateChecklistResponse(retryContent, country, visaType, attempt);
                ...
                if (!validationResult.isValid) {
                    (0, logger_1.logError)('[OpenAI][Checklist] Attempt 2 also failed validation, triggering fallback', new Error('Validation failed after retry'), {
                        country,
                        visaType,
                        errors: validationResult.errors,
                        warnings: validationResult.warnings,
                    ...
                    (0, logger_1.logInfo)('[OpenAI][Checklist] Attempt 2 succeeded after retry', {
                        country,
                        visaType,
            ...
            if (!parsed || !validationResult.isValid) {
                (0, logger_1.logError)('[OpenAI][Checklist] Both attempts failed, using fallback checklist', new Error('GPT-4 validation failed'), {
                    country,
                    visaType,
                ...
                const countryCode = countryCodeMap[country.toLowerCase()] || country.substring(0, 2).toUpperCase();
                const normalizedVisaType = visaType.toLowerCase().includes('student')
                    ? 'student'
                    : 'tourist';
                const fallbackItems = getFallbackChecklist(countryCode, normalizedVisaType);
                parsed = {
                    type: visaType,
                    country: country,
                ...
                (0, logger_1.logInfo)('[OpenAI][Checklist] Using fallback checklist', {
                    country,
                    visaType,
                ...
                if (validationResult.warnings.some((w) => w.includes('Missing'))) {
                    (0, logger_1.logInfo)('[OpenAI][Checklist] Auto-translating missing translations');
                    await autoTranslateChecklistItems(parsed.checklist);
                }
                // Auto-correct any remaining issues
                const { autoCorrectChecklist } = await Promise.resolve().then(() => __importStar(require('../utils/json-validator')));
                parsed = autoCorrectChecklist(parsed, country, visaType);
                (0, logger_1.logInfo)('[OpenAI][Checklist] Validation passed after corrections', {
                    country,
                    visaType,
            ...
            (0, logger_1.logInfo)('[OpenAI][Checklist] Checklist generation completed', {
                model: this.MODEL,
                country,
                visaType,
                tokensUsed: totalTokens,
            ...
            if (!parsed || !parsed.checklist || !Array.isArray(parsed.checklist)) {
                (0, logger_1.logError)('[OpenAI][Checklist] Final validation failed - checklist is missing or invalid, using fallback', new Error('Invalid checklist format from AI: missing or invalid checklist array'), {
                    country,
                    visaType,
                    hasParsed: !!parsed,
                ...
                (0, logger_1.logInfo)('[OpenAI][Checklist] Using emergency fallback checklist', {
                    country,
                    visaType,
                    itemCount: fallbackItems.length,
                });
            ...
            if (parsed.checklist.length === 0) {
                (0, logger_1.logError)('[OpenAI][Checklist] Checklist is empty, using fallback', new Error('AI returned empty checklist array'), {
                    country,
                    visaType,
                });
            ...
            const { MIN_ITEMS_HARD, IDEAL_MIN_ITEMS, MAX_ITEMS_HARD, IDEAL_MAX_ITEMS } = await Promise.resolve().then(() => __importStar(require('../config/checklist-config')));
            const itemCount = parsed.checklist.length;
            if (itemCount < MIN_ITEMS_HARD) {
                (0, logger_1.logWarn)('[OpenAI][Checklist] AI returned too few items, minimum required', {
                    country,
                    visaType,
                    itemCount,
                ...
                (0, logger_1.logWarn)('[OpenAI][Checklist] AI returned fewer than ideal items', {
                    country,
                    visaType,
                    itemCount,
            ...
            if (parsed.checklist.length > IDEAL_MAX_ITEMS) {
                (0, logger_1.logWarn)('[OpenAI][Checklist] AI returned more than recommended items', {
                    country,
                    visaType,
                    itemCount: parsed.checklist.length,
            ...
            const { ensureCategoryConsistency } = await Promise.resolve().then(() => __importStar(require('../utils/checklist-helpers')));
            ...
            (0, logger_1.logError)('[OpenAI][Checklist] Checklist generation failed', error instanceof Error ? error : new Error(String(error)), {
                model: this.MODEL,
                country,
                visaType,
            ...
            const { ensureCategoryConsistency } = await Promise.resolve().then(() => __importStar(require('../utils/checklist-helpers')));
            const isStudent = visaType.toLowerCase().includes('student') || visaType.toLowerCase().includes('study');
    ...
    static async generateChecklistLegacy(application, aiUserContext) {
        ...
        (0, logger_1.logInfo)('[OpenAI][Checklist][Legacy] Generating checklist with structured output', {
            country,
            visaType,
            countryCode,
        });
        ...
        try {
            ...
            const { getVisaKnowledgeBase } = await Promise.resolve().then(() => __importStar(require('../data/visaKnowledgeBase')));
            const { getRelevantDocumentGuides } = await Promise.resolve().then(() => __importStar(require('../data/documentGuides')));
            const { MIN_ITEMS_HARD, IDEAL_MIN_ITEMS } = await Promise.resolve().then(() => __importStar(require('../config/checklist-config')));
            // Get visa knowledge base and document guides
            const visaKb = getVisaKnowledgeBase(country, visaType.toLowerCase());
...
AIOpenAIService.MODEL = process.env.OPENAI_MODEL || 'gpt-4o-mini';
// Checklist generation ALWAYS uses GPT-4 (gpt-4o or gpt-4.1) - never falls back to mini
AIOpenAIService.CHECKLIST_MODEL = process.env.OPENAI_MODEL_CHECKLIST || 'gpt-4o';
