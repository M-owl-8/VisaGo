...
3. Copies the appropriate schema:
   - `schema.postgresql.prisma` → `schema.prisma` (if PostgreSQL)
   - `schema.sqlite.prisma` → `schema.prisma` (if SQLite)
...
**PostgreSQL (Railway):**
Use Railway dashboard → Database → Data tab
...
- ✅ Railway (PostgreSQL)
- ✅ Heroku (PostgreSQL)
- ✅ Vercel (PostgreSQL with connection pooling)
- ✅ AWS RDS (PostgreSQL)
- ✅ Docker (both SQLite and PostgreSQL)

Just set `DATABASE_URL` and it auto-configures! 🚀
