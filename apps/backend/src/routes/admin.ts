 ...
 * Get user details
 */
router.get("/users/:userId", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * Update user role
 */
router.patch("/users/:userId/role", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { role } = req.body;
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const application = await AdminService.updateApplicationStatus(req.params.applicationId, status);
 ...
 */
router.get("/documents/verification-queue", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const skip = parseInt(req.query.skip as string) || 0;
    const take = Math.min(parseInt(req.query.take as string) || 20, 100);
 ...
 */
router.patch("/documents/:documentId/verify", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const document = await AdminService.updateDocumentStatus(req.params.documentId, status, notes);
 ...
 */
router.get("/analytics/metrics", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get("/analytics/conversion-funnel", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get("/analytics/user-acquisition", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * Get event breakdown
 */
router.get("/analytics/events", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const days = parseInt(req.query.days as string) || 30;
