#!/usr/bin/env node
/**
 * AI Evaluation CLI - Document Check Subsystem
 */

import { runDocCheckEval } from '../runner';
import { printFullReport, saveJsonReport } from '../reporters';

async function main() {
  console.log('🚀 Starting AI Evaluation - Document Check Subsystem\n');
  ...
  try {
    const results = await runDocCheckEval(options);

    printFullReport(results);

    const reportPath = process.env.REPORT_PATH || './ai-eval-doccheck-report.json';
