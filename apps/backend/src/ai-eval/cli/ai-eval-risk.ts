#!/usr/bin/env node
/**
 * AI Evaluation CLI - Risk Explanation Subsystem
 */

import { runRiskEval } from '../runner';
import { printFullReport, saveJsonReport } from '../reporters';

async function main() {
  console.log('🚀 Starting AI Evaluation - Risk Explanation Subsystem\n');
  ...
  try {
    const results = await runRiskEval(options);

    printFullReport(results);

    const reportPath = process.env.REPORT_PATH || './ai-eval-risk-report.json';
