/**
 * Checklist Training Data Mapper
 * Maps AIInteraction records to TrainingExample for checklist enrichment
 */

import { TrainingExample, AITrainingMeta } from './types';

// AIInteraction type from Prisma (after migration, model name will be aIInteraction in camelCase)
type AIInteraction = {
  id: string;
  taskType: string;
  model: string;
...
export function mapChecklistInteractionToTrainingExample(
  ...
  try {
    ...
    // Build input (essential context)
    const input = {
      baseDocuments: requestPayload.baseDocuments || [],
      canonicalAIUserContext: requestPayload.canonicalAIUserContext || {},
      embassySummary: requestPayload.embassySummary || null,
      countryCode: requestPayload.countryCode,
      visaType: requestPayload.visaType,
    };

    // Build output (the checklist)
    ...
    console.error('[Mapper] Failed to map checklist interaction', {
