...
describe('Priority 2: AI Chat System Completion (Simplified)', () => {
  ...
  describe('1. Message Persistence & Retrieval', () => {
    ...
    test('should persist assistant response with sources', async () => {
      const sources = [{ source: 'doc1.pdf', snippet: 'Information about visa types' }];
  ...
  describe('2. Conversation Context (Last 10 Messages)', () => {
    ...
    test('should respect limit parameter', async () => {
      const limitedHistory = await chatService.getConversationHistory(
        TEST_SESSION_ID,
        undefined,
        1,
        0
      );
      const fullHistory = await chatService.getConversationHistory(
        TEST_SESSION_ID,
        undefined,
        50,
        0
