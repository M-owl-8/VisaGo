...
/**
 * Evaluate a condition string against an AIUserContext.
...
export function evaluateCondition(condition: string | undefined, ctx: EvalContext): boolean {
  ...
  const vars: Record<string, any> = {
    sponsorType: ctx.user.questionnaireSummary?.sponsorType,
    currentStatus: ctx.user.questionnaireSummary?.currentStatus,
    employmentStatus: ctx.user.questionnaireSummary?.currentStatus,
    isStudent: ctx.user.questionnaireSummary?.isStudent,
    hasUniversityInvitation: ctx.user.questionnaireSummary?.hasUniversityInvitation,
    hasSponsor: ctx.user.questionnaireSummary?.isSponsored,
    travelHistoryLevel: ctx.user.questionnaireSummary?.travelHistoryLevel,
    riskLevel: ctx.user.riskScore,
  };

  // Very small, controlled evaluator using Function constructor with sanitized identifiers
  try {
    ...
    logWarn('[ConditionEvaluator] Failed to evaluate condition, defaulting to true', {
      condition,
