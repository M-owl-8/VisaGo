...
export const usePaymentStore = create<PaymentStore>()(
  persist(
    (set, get) => ({
          ...
          if (response.success && response.data) {
            ...
            if (response.frozen) {
              set({ 
                ...
                freezeStatus: {
                  isFrozen: true,
                  message: response.message || "Payments are currently free!",
                  freezeEndDate: response.freezeEndDate,
      ...
      loadFreezeStatus: async () => {
        try {
          const response = await apiClient.getPaymentFreezeStatus();
          if (response.success && response.data) {
            set({ freezeStatus: response.data });
          }
        } catch (error: any) {
          console.error("Load freeze status error:", error);
        }
      },
      ...
      initiatePayment: async (applicationId, returnUrl, paymentMethod = "payme") => {
        ...
        try {
          ...
          if (response.success && response.data) {
            // Check if payment is frozen (free period)
            if (response.data.frozen || response.paymentRequired === false) {
              // Payment is free, return success without payment URL
              return {
                transactionId: `free_${Date.now()}`,
                paymentUrl: "",
                frozen: true,
                message: response.data.message || "Payment is free during the trial period!",
              } as any;
            }
